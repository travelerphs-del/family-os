...
## Session 7.2 — Travel 다음 작업 결정만 (코드 작업 X)

**기간**: 2026-05-17 (Session 7.1 직후)
**범위**: 사용자가 짚은 3가지 한계에 대한 결정. 코드 작업은 다음 세션으로.

### [DECISION] 3가지 확정

1. **가족 멤버 8명 확장**: 부모 → 자녀 → 친조부모 → 외조부모. Travel 만 사용. Health 영향 X.

2. **호텔 처리 = 옵션 C** (stay_start/stay_end 컬럼 추가):
   - places 시트에 두 컬럼 추가 (13 → 15)
   - 호텔만 stay range, 다른 카테고리는 기존 visited_date
   - 필터 칩: 호텔은 체류 기간 내 모든 일자에서 표시

3. **검색 = A+C 콤보** (Google Places + 지도 클릭 폴백):
   - Mapbox 의 동아시아 POI 부족 인정 (대만/한국/일본)
   - 검색만 Google Places API (New), 지도는 Mapbox 유지
   - 검색 실패 시 폴백: 지도 직접 클릭 → 좌표 캡쳐 + 사용자 이름 직접 입력

### [LESSON]

1. **사용자 케이스 정확히 읽기**: 사용자는 "대만" 호텔 검색이 안 된다고 했는데 나는 "한국 POI" 위주로 답함. 결론은 같지만 (Google이 다 강함) 답변 정확성 부족. 미안.
2. **Phase B 결정 시 trade-off 강조 부족**: "한국 섞이면 Google 이 나음" 정도가 아니라 "동아시아 여행 중심이면 Mapbox 검색은 자주 실패할 것" 이라고 단정적으로 짚었어야 했음.
3. **결정은 context 파일에 박아두면 됨**: 다음 세션에 context-travel.md 첨부하면 결정 다시 설명 불필요. archive 는 굳이 첨부 X.

### [PENDING]

- [다음 세션] context-travel.md §3 의 3가지 작업 (가족 멤버 8명, 호텔 stay_range, Google Places 통합)
- 사용자 사전 작업: Google Cloud 프로젝트 + Places API 활성화 + API key 발급 + daily quota cap 100

### [COST]

- 운영 비용: 변경 없음. Google Places 추가 시도 가족 사용량으로는 한도의 1% 이내.
- 작업 토큰: 거의 0 (문서만)
